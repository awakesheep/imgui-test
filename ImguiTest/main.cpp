...
int main(int, char**)
{
	...
	GLFWwindow* window = glfwCreateWindow(1280, 720, "ImGuiTest", NULL, NULL);
	if (window == NULL)
		return 1;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		static char buf1[64] = ""; ImGui::InputText(u8"加数1", buf1, 64, ImGuiInputTextFlags_CharsDecimal);
		static char buf2[64] = ""; ImGui::InputText(u8"加数2", buf2, 64, ImGuiInputTextFlags_CharsDecimal);
		...
		{
			ImGui::Text(u8"计算结果为：%d",add1+add2);
			ImGui::Separator();
			...
			ImGui::Checkbox(u8"下次不要再提示我", &dont_ask_me_next_time);
			ImGui::PopStyleVar();
